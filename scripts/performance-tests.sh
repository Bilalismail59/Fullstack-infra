...
# Fonction pour tester la latence réseau
test_network_latency() {
    echo -e "${BLUE}=== TESTS DE LATENCE RÉSEAU ===${NC}"
    
    # Test de latence entre les services
    ...
    echo ""
    echo "Test de latence frontend -> mysql:"
    kubectl exec -n default deployment/frontend-deployment -- ping -c 5 mysql.default.svc.cluster.local 2>/dev/null || echo "Test de ping échoué"
}
...
test_resilience() {
    ...
    # Vérifier les politiques de redémarrage
...
test_scalability() {
    ...
    # Vérifier les HPA (Horizontal Pod Autoscaler) s'ils existent
...
# 1. Tests de performance des services
...
# 2. Tests des ressources système
test_system_resources
